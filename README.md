...
2. 将内容转换成“内容主题、原子主张、新闻事实、隐性观点”标准协议；
...
```json
{
  ...
  "新闻事实": [
    "视频围绕市场上出现的白籽西瓜和无籽西瓜讨论其培育方式与食品安全"
  ],
  "隐性观点": [
    "人工干预培育出的农作物不安全"
  ]
...
结构不是依赖提示词模拟：MiMo 请求使用 `response_format=json_schema` 和 `strict=true`，服务端再使用 Pydantic 以同一协议验证字段、类型、单条长度、中文完整句和去重。三类数组不设固定数量上限；系统根据原文长度、视频时长和图文页数计算“自适应语义密度”软预算，引导模型先按核验事件聚类，再用最小充分命题覆盖关键核验路径。软预算不进入 Schema、不截断数组；独立核心事件可以超过建议值。服务端质量门会检测短句流水账、非自足代词、高重叠条目和跨条重复数字，只有重压缩结果风险严格下降时才接受替换。截断、语法错误或 Schema 错误仍会触发受约束修复；最终失败时保留完整原文和合法降级结果。
...
- “新闻事实”表示内容中明确陈述的事实性背景，不表示本节点已经核真。
